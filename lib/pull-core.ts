...
export function computeWeekBuckets(
  ...
  // The axis is always a trailing period ending at the pull, never the sample's
  // own min/max dates. An `all` pull reaches back years, so deriving the span
  // from the data let a single old post stretch the axis across ~4 years - each
  // "week" became ~10 months and ~90% of every subject's engagement landed in
  // the final bucket. `all` therefore charts the same trailing 30 days as a 30d
  // pull; older posts still count in Share of Voice, they just aren't plotted.
  const end = generatedAtMs;
  const start = end - (window === "7d" ? 7 : 30) * DAY_MS;
