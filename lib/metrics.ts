...
/**
 * Reach-normalized response rates. Unlike the raw totals these are ratios, so
 * they stay comparable no matter how many records a pull returned or when those
 * posts were published - which is what makes them safe to chart against an
 * all-time sample, where record counts and publish dates are set by relevance
 * ranking rather than by a fixed period.
 */
export function commentsPerMillionViews(m: EntityMetrics): number {
  return m.views === 0 ? 0 : m.comments / (m.views / 1_000_000);
}

/** Shares per million views - how hard the content pushes itself outward. */
export function sharesPerMillionViews(m: EntityMetrics): number {
  return m.views === 0 ? 0 : m.shares / (m.views / 1_000_000);
}
...
export interface EntityStats {
  ...
  commentsPerMillionViews: number;
  sharesPerMillionViews: number;
}
  ...
  return Object.fromEntries(
    entities.map((e) => [
      e.id,
      {
        ...
        commentsPerMillionViews: commentsPerMillionViews(e.metrics),
        sharesPerMillionViews: sharesPerMillionViews(e.metrics),
      },
    ]),
  );
}
